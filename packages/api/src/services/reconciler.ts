...
import { listPending, markSubmitted, recordFailure, markDeadLettered } from '../store/escalation-outbox.ts'
import { getSession } from '../store/sessions.ts'

// Drains the escalation outbox into Zendesk once it recovers. Re-submits each pending record with its
...
// give up after bounded attempts so a permanently-broken escalation dead-letters instead of looping
...
export const runReconcilerOnce = async (): Promise<void> => {
  for (const record of listPending()) {
    try {
      ...
      markSubmitted(record.idempotencyKey, ticket.id)
      ...
      if (record.attempts + 1 >= MAX_ATTEMPTS) markDeadLettered(record.idempotencyKey, err.message)
      else recordFailure(record.idempotencyKey, err.message)
    }
  }
}
