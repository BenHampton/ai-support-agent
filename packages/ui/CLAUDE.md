...
- **MUI wrappers are still CSS Modules** — `App*` primitives wrap MUI but are styled with a co-located `.module.css` using `vars.css` tokens, not `sx`/inline. Reach MUI's internals through its **global slot classes** scoped under our class: `.select :global(.MuiSelect-select) { … }`. Portalled slots (e.g. the menu Paper) get our class via `slotProps` — `MenuProps={{ slotProps: { paper: { className: styles.menu } } }}`. `StyledEngineProvider injectFirst` (set once in `main.tsx`) injects MUI's styles first so our CSS Modules reliably win by source order; a minimal MUI dark `ThemeProvider` (`src/theme.ts`, mirrors `vars.css`) gives MUI's unstyled defaults the right palette.
- **NEVER use `!important`** — win by specificity instead. When a MUI rule outranks ours, scope our class with the MUI global class the element already carries to raise specificity above it (e.g. `.error:global(.MuiAlert-root)`, `.tooltip:global(.MuiTooltip-tooltip)`, `.select:global(.MuiInputBase-root)::before`). For higher-specificity MUI states (e.g. the hover underline), add an explicit matching state rule (`.select:global(.MuiInputBase-root):hover::before`). `!important` is banned in all component and global CSS.
...
- **Reusable components — `App`-prefixed, flat under `src/components/`** — cross-page primitives (plus the `NavBar` shell) live directly at `components/<AppName>/<AppName>.tsx` + `.module.css` with an `App` prefix (e.g. `components/AppSelectOptions/`), distinct from the `pages/` screens. Some wrap a **MUI** primitive so the rest of the app imports the wrapper, never MUI directly (`AppSelectOptions`, `AppTooltip`, `AppAlert`); others are plain themed elements where MUI adds no behavior (`AppBadge`, `AppButton`). Props are a minimal, app-shaped local `type Props` (e.g. `value`/`onChange`/`options`/`disabled`/`ariaLabel`) — not MUI's API surface.
